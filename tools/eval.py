...
from toolkit.datasets import OTBDataset, UAVDataset, LaSOTDataset, \
        VOTDataset, NFSDataset, VOTLTDataset, LMDTShipDataset
...
def main():
    ...
    if 'OTB' in args.dataset:
        ...
        benchmark.show_result(f1_result,
                show_video_level=args.show_video_level)
    elif 'LMDTSHIP' == args.dataset:
        dataset = LMDTShipDataset(args.dataset, root)
        dataset.set_tracker(tracker_dir, trackers)
        benchmark = OPEBenchmark(dataset)
        success_ret = {}
        with Pool(processes=args.num) as pool:
        ...
        overlap_ret = {}
        with Pool(processes=args.num) as pool:
            for ret in tqdm(pool.imap_unordered(benchmark.eval_overlap, trackers),
                            desc='eval overlap', total=len(trackers), ncols=100):
                overlap_ret.update(ret)
        distance_ret = {}
        with Pool(processes=args.num) as pool:
            for ret in tqdm(pool.imap_unordered(benchmark.eval_distance, trackers),
                            desc='eval distance', total=len(trackers), ncols=100):
                distance_ret.update(ret)
        benchmark.show_result(success_ret, precision_ret, norm_precision_ret,
                overlap_ret=overlap_ret, distance_ret=distance_ret,
                show_video_level=args.show_video_level)
