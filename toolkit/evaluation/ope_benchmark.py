...
from ..utils import success_overlap, success_error, average_overlap, average_distance
...
class OPEBenchmark:
    ...
    def eval_overlap(self, eval_trackers=None):
        ...
        for tracker_name in eval_trackers:
            ...
            for video in self.dataset:
                ...
                overlap_ret_[video.name] = average_overlap(gt_traj, tracker_traj, n_frame)
            overlap_ret[tracker_name] = overlap_ret_
        return overlap_ret

    def eval_distance(self, eval_trackers=None):
        if eval_trackers is None:
            eval_trackers = self.dataset.tracker_names
        if isinstance(eval_trackers, str):
            eval_trackers = [eval_trackers]
        ...
        for tracker_name in eval_trackers:
            ...
            for video in self.dataset:
                ...
                distance_ret_[video.name] = average_distance(gt_center, tracker_center, n_frame)
            distance_ret[tracker_name] = distance_ret_
        return distance_ret
    ...
    def show_result(self, success_ret, precision_ret=None,
            norm_precision_ret=None, overlap_ret=None, distance_ret=None,
        ...
        results = []
        columns = ["Tracker name", "Success", "Norm Precision", "Precision", "Avg Overlap", "Avg Distance"]
        tracker_name_len = max((max([len(x) for x in success_ret.keys()])+2), 12)
        header = ("|{:^"+str(tracker_name_len)+"}|{:^9}|{:^16}|{:^11}|{:^13}|{:^14}|").format(*columns)
        formatter = "|{:^"+str(tracker_name_len)+"}|{:^9.3f}|{:^16.3f}|{:^11.3f}|{:^13.3f}|{:^14.3f}|"
        print('-'*len(header))
        print(header)
        print('-'*len(header))
        for tracker_name in tracker_names:
            ...
            if overlap_ret is not None:
                average_overlap = np.mean(list(overlap_ret[tracker_name].values()))
            if distance_ret is not None:
                average_distance = np.mean(list(distance_ret[tracker_name].values()))
            results.append([tracker_name, success, norm_precision, precision, average_overlap, average_distance])
            print(formatter.format(tracker_name, success, norm_precision, precision, average_overlap, average_distance))
        print('-'*len(header))

        if show_video_level:
            success_ret = {k: success_ret[k] for k in tracker_names[:1]}
            video_results = []
            video_columns = ['Video', 'Success', 'Precision', 'Overlap', 'Distance']
            print("\n\n")
            ...
            for video in videos:
                ...
                for tracker_name in success_ret.keys():
                    ...
                    overlap = np.mean(overlap_ret[tracker_name][video])
                    distance = np.mean(distance_ret[tracker_name][video])
                    ...
                    if precision < helight_threshold:
                        ...
                        row += precision_str+'|'
                    video_results.append([video, success, precision, overlap, distance])
                print(row)
            print('-'*len(header1))
