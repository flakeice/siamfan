...
def average_overlap(gt_bb, result_bb, n_frame):
    iou = np.ones(len(gt_bb)) * (-1)
    mask = np.sum(gt_bb[:, 2:] > 0, axis=1) == 2
    iou[mask] = overlap_ratio(gt_bb[mask], result_bb[mask])
    assert len(iou) == n_frame
    average_iou = np.sum(iou) / float(n_frame)
    return average_iou

def average_distance(gt_center, result_center, n_frame):
    ...
    assert len(dist) == n_frame
    average_dist = np.sum(dist) / float(n_frame)
    return average_dist
